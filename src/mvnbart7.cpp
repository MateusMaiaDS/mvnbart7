                ...
                for(int j = 0; j < data.y_mat.n_cols; j++){
                        ...
                        arma::mat Sigma_calculation_aux = (Sigma_mj_j.t()*Sigma_mj_mj_inv); // This line calculate a quantitiy that is constant for the i_train so avoids multiple calculations;
                                ...
                                for(int i_train = 0; i_train < data.y_mat.n_rows;i_train++){
                                                // Adding a possibility to get y.col = 1
                                                if(data.y_mat.n_cols==1){
                                                        partial_u(i_train) = 0.0; // Old version
                                                }else {
                                ...
                                double v;
                                if(data.y_mat.n_cols==1){
                                        v = Sigma_j_j - arma::as_scalar(Sigma_j_mj*Sigma_mj_mj_inv*Sigma_mj_j);
                                } else {
                                        v = Sigma_j_j;
                                }
...
// CPP-BART function
// [[Rcpp::export]]
Rcpp::List cppbart_uni(arma::mat x_train,
                   arma::mat y_mat,
                   arma::mat x_test,
                   arma::mat x_cut,
                   int n_tree,
        ...
        for(int i = 0;i<data.n_mcmc;i++){
                ...
                arma::vec partial_u(data.x_train.n_rows,arma::fill::zeros);
...
// Generating the sampler univaariate

double up_tn_sampler_uni(arma::mat &z_mat_, arma::mat &mean_mat_, double lower, double v_j_,
                     int i_, int j_, bool tn_sampler){
        ...
        // Getting the shed vesion
        double mean_ = mean_mat_(i_,j_); // Old version

        if(tn_sampler){
                return truncated_sample(mean_,true,sqrt(v_j_)); // I could even simplify more setting the tree equal to one
        } else {
                while(sample_bool){
...
double lw_tn_sampler_uni(arma::mat &z_mat_, arma::mat &mean_mat_, double upper, double v_j_,
                     int i_, int j_, bool tn_sampler){
...
// Updating Z
void update_z_uni(arma::mat &z_mat_,
              arma::mat &y_hat,
              modelParam &data,
              int j_,
              bool tn_sampler){
        ...
        // Veriying v_j
        if(data.v_j!=1.0){
                Rcpp::stop("Error the correlation should be equal to one");
        }
        ...
        for(int i = 0; i < data.x_train.n_rows; i++){
                ...
                if(data.y_mat(i,j_)==1){
                        // cout << "Y_hat(" <<i<<","<<j_<<") :" << y_hat(i,j_) << endl;
                        z_mat_(i,j_) = up_tn_sampler_uni(z_mat_,y_hat,0.0,data.v_j,
                               i,j_,tn_sampler);
                } else {
                        z_mat_(i,j_) = lw_tn_sampler_uni(z_mat_,y_hat,0.0,data.v_j,
                               i,j_,tn_sampler);
                }
        }
}
...
// [[Rcpp::export]]
Rcpp::List cppbart_CLASS_uni(arma::mat x_train,
                         arma::mat y_mat,
                         arma::mat x_test,
                         arma::mat x_cut,
                         int n_tree,
        ...
        for(int i = 0;i<data.n_mcmc;i++){
                ...
                for(int j = 0; j < data.y_mat.n_cols; j++){
                        ...
                        // Updating z_j values
                        // Rcpp::Rcout << "Error on update z" << endl;
                        update_z_uni(z_mat_train,y_mat_hat,data,j,tn_sampler);
                        // Rcpp::Rcout << "Sucess!" << endl;

...
mvnbart <- function(x_train,
     ...
     if(class_model){
             ...
             S_0_wish <- 2*df*diag(c(rate_tau),nrow = ncol(y_mat_scale))

             # Call the bart function
             if(is.null(Sigma_init)){
                     Sigma_init <- diag(nsigma^2, ncol = ncol(y_mat_scale))
             }
     ...
     if(class_model){
                if(ncol(y_mat) > 1 ){
                        ...
                        bart_obj <- cppbart_CLASS_uni(x_train_scale,
                                                  y_mat,
                                                  x_test_scale,
                                                  xcut_m,
                                                  n_tree,
             ...
             if(ncol(y_mat_scale)>1){
                bart_obj <- cppbart(x_train_scale,
                     ...
                     bart_obj <- cppbart_uni(x_train_scale,
                                         y_mat_scale,
                                         x_test_scale,
                                         xcut_m,
                                         n_tree,
     ...
     Sigma_scale <- diag((max_y-min_y), ncol = ncol(y_mat))

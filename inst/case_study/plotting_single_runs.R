...
library(ggplot2)
mvbart_mod <- readRDS("~/Documents/lab_spBART/mvnbart6/inst/case_study/rep_1_single_run1000_ntree_50_mvn_2_task_classification_sim_friedman1_nmcmc_,10000.Rds")
...
rho12_post_plot <- ggplot()+
     geom_line(data = data.frame(n_post = 1:length(sigma1_post),
                                 sigma1 = rho12_post),
               mapping = aes(x = n_post, y = sigma1), col = c("#B51700"), alpha = 0.5)+
     geom_hline(yintercept = 0.75, linetype = 'dashed', col = 'black')+
     xlab("Posterior iter")+
     ylab(expression(rho[12]))+
     theme_bw()+
     theme(text = element_text(size = 14))  # Adjust the size as needed

cowplot::plot_grid(sigma1_post_plot,sigma2_post_plot,rho12_post_plot,
                   nrow = 3)
rho12_post_plot
...
cowplot::plot_grid(varimport_plots[[1]],varimport_plots[[2]], ncol = mvn_dim)
